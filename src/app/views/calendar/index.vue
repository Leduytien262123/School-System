...
const compData = reactive({
  ...
  format(year, month, date) {
    ...
    return `${dd.gzYear} ${dd.IMonthCn} ${dd.IDayCn} <br/> ${dd.Animal} ${
      dd.astro
    } <br/> ${dd.lunarFestival ? dd.lunarFestival : ""}`;
  },
...
updateText(
  new Date(compData.value).getFullYear(),
  new Date(compData.value).getMonth(),
  new Date(compData.value).getDay()
);
</script>
