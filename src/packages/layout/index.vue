  ...
  <n-el tag="div" class="flex flex-col" style="height: 100%">
    ...
    <n-layout fle has-sider style="flex: 1">
      <n-layout-sider
        ...
        :show-trigger="
          $globalStore.configs.layoutName === 'default' &&
          !$globalStore.deviceInfo.isMobile
        "
        collapse-mode="width"
        :collapsed-width="
          $globalStore.configs.layoutName === 'lessen'
            ? $globalStore.configs.layoutSiderWidth
            : $globalStore.configs.collapsedWidth
        "
        :width="
          $globalStore.deviceInfo.isMobile
            ? 0
      ...
      <div class="flex h-full flex-col flex-auto overflow-hidden item">
        <template v-if="components.LayoutHeaderCum">
          <template
            v-if="
              $globalStore.configs.isBreadcrumb && $globalStore.appGroups.length
            "
          >
    ...
    <n-drawer
      v-model:show="$globalStore.configs.mobileMenuVisible"
      :width="$globalStore.configs.mobileMenuDrawer"
      placement="left"
    >
      <n-drawer-content
        body-content-style="padding: 0;"
        :title="$globalStore.configs.title"
      >
        <SiderMenu />
      </n-drawer-content>
    </n-drawer>
