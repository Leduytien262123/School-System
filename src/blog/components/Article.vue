    ...
    <div class="item">
      <div class="item-img">
        ...
        <a
          class="background"
          href="javascript:"
          :style="{ backgroundImage: 'url(' + itemData.pic + ')' }"
          v-else
        ></a>
      ...
      <div class="item-userimg">
        <img
          src="https://www.vipbic.com/uploads/20190321/e561b9dbf43336c22e277169027c8add.jpg"
          :alt="itemData.title"
        />
      </div>
      <div class="item-meta">
        <div>
          <span class="d"></span
          ><span class="t">{{ itemData.category.name }}</span>
        </div>
        <template v-for="(todo, index) in itemData.keywords">
          <div :key="index" v-if="index < 1">
            <span class="d"></span><span class="t">{{ todo }}</span>
          </div>
        </template>
      </div>
      <div class="item-body">
      ...
      <div class="item-det">
        <a class="btn" target="_blank" @click="handleVisible">Xem chi tiết</a>
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const formatTitle = () => {
      return `Tác giả: ${props.itemData.member.username} ,Tiêu đề: ${
        props.itemData.title
      } ,Từ khóa: ${props.itemData.keywords.join(
        ","
      )} ,Ngày đăng: ${gzYear}[${animal}] ${gzMonth}月 ${gzDay}日 ${ncWeek}`;
    };
