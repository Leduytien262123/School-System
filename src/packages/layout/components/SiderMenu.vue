<template>
  ...
  <n-el tag="div" class="h-full flex overflow-hidden w-full" v-else>
    <ul
      :style="{ maxWidth: $globalStore.configs.layoutSiderWidth }"
      class="min-w-[90px] flex-1 p-2 box-border border-l border-r border-solid bag-border-color h-full"
    >
      <li
        @click="handleMenuItemClick($global, item)"
        :class="[
          'menu-item truncate text-balance transition text-center first:mt-0 py-2 last:mb-0 flex flex-col items-center mb-3 mt-3 cursor-pointer rounded-md',
          classNameMenuItemActive($globalStore, item),
        ]"
...
const classNameMenuItemActive = ($globalStore, item) => {
  if (
    $globalStore.currentRouter.meta.topId === item.id ||
    $globalStore.currentRouter.meta.id === item.id
  ) {
    return "active";
  }
};
const getCurrentRouterId = computed(
  () =>
    Number($globalStore.currentRouter.query.rid) ||
    Number($globalStore.currentRouter.meta.id)
);
