    ...
    <n-page-header>
      <template #title>
        <component is="LayoutHeaderTitle">
          ...
          >
            <img
              class="h-[32px] object-cover max-w-full pr-2"
              v-if="$globalStore.webSite.logo"
              :src="$globalStore.webSite.logo"
              :alt="$globalStore.webSite.title"
            />
            <a href="/" class="text-base flex-shrink-0 font-bold">{{
              $globalStore.webSite.title
            }}</a>
            <span class="text-xs ml-2 flex-shrink-0">{{
              $globalStore.webSite.subTitle
            }}</span>
          </div>
          <div
          ...
          >
            <n-button
              @click="
                $globalStore.configs.mobileMenuVisible =
                  !$globalStore.configs.mobileMenuVisible
              "
              :bordered="false"
            >
      ...
      <template #extra>
        <div class="flex items-center">
          <component is="LayoutHeaderExtra">
            <n-dropdown
              trigger="click"
              v-for="item in compData.allOptions"
