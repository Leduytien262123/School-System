<template>
  <n-modal
    v-if="$globalStore.configs.formStyle === 'dialogue'"
    :mask-closable="false"
...
watch(props, () => {
  if (
    props.showModal &&
    globalProperties.$globalStore.configs.formStyle === "tab"
  ) {
    const title =
      globalProperties.$global?.helpers?.formatTitle(
        globalProperties.$global,
        globalProperties.$route.meta
      ) +
      "-" +
      attrs?.modal?.title;
    ...
    globalProperties.$router.addRoute(
      globalProperties.$route.meta.root
        ? globalProperties.$route.meta.root
        : "layout",
      {
        component,
