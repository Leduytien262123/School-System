<template>
  <n-el
    tag="div"
    class="tab-custom flex flex-shrink-0 border-solid border-b bag-border-color items-center"
  >
    <n-icon
      @click="handleScroll('l')"
      size="20"
      class="px-2 w-auto flex-shrink-0 cursor-pointer"
    >
      <ChevronBackOutline></ChevronBackOutline>
    </n-icon>
    <n-scrollbar trigger="none" ref="scrollbar" x-scrollable>
      <n-el
        v-if="$globalStore.configs.tabStyle === 'button'"
        tag="div"
      ...
      >
        <n-space :wrap="false">
          <n-button
            :color="
              $globalStore.currentRouter.path === item.path
          ...
          >
            ...
            <template v-if="!item.hasClose" #icon>
              <n-icon
                size="18"
                @click.stop="$global?.helpers?.closeTabBarJump($global, item)"
              >
                <CloseOutline />
              </n-icon>
            </template>
      ...
      <n-el
        v-if="$globalStore.configs.tabStyle === 'card'"
        tag="div"
      ...
      >
        <n-space :wrap="false">
          <div
            :class="[
              'flex px-3 py-2 items-center justify-center cursor-pointer',
            ]"
...
const handleSelectOptions = ($global, $globalHook, $globalStore, key) => {
  ...
  if (key === "closeCurrentTab") {
    const data = $globalStore.tabs.find(
      (item) => item.id === $globalStore.currentRouter.meta.id
    );
    data && $global?.helpers?.closeTabBarJump($global, data);
  }
  if (key === "closeLeftTab") {
    const index = $globalStore.tabs.findIndex(
      (item) => item.id === $globalStore.currentRouter.meta.id
    );
  ...
  if (key === "modalWindowTab") {
    createWindow({
      title: $global.helpers.formatTitle(
        $global,
        $globalStore.currentRouter.meta
      ),
