<template>
  <n-grid cols="12" y-gap="20" x-gap="20" item-responsive responsive="screen">
    <n-grid-item span="12 m:12 l:12">
      ...
      >
        <template #header-extra>
          Lậu Thất Minh, nhà Đường: Lưu Vũ Tích
        </template>
        <template v-for="item in compData.items1">
          <div
            class="inline-block text-center w-[42px]"
            :key="idx"
            v-for="(todo, idx) in item"
          >
            <span class="clear-both text-[12px] float-left w-[42px]">{{
              todo.py
            }}</span>
            <span
              class="clear-both mb-2 text-center float-left text-[16px] h-[36px] w-[42px]"
              >{{ todo.hz }}</span
            >
          </div>
          <br />
    ...
    <n-grid-item span="12 m:12 l:12">
      ...
      >
        <template #header-extra>
          Tiểu truyện Lý Hạ, Lý Thương Ẩn [nhà Đường]
        </template>
...
const text2 = [
  ...
  "  长吉将死时，忽昼见一绯衣人，驾赤虬，持 một bảng, sách若太古篆 hoặc霹雳石文者, 云当召长吉. 长吉了不能 đọc, 欻下榻叩头, 言: “阿㜷老且病, 贺不愿去.” 绯衣人笑曰: “帝成白玉楼, 立召君为记. 天上差乐, 不苦也.” 长吉独泣, 边人尽见之. 少之, 长吉气绝. 常所居窗中, 勃勃有烟气, 闻行车嘒管之声. 太夫人急止人哭, 待之如炊五斗黍许时, 长吉竟死. 王氏姊非能造作谓长吉者, 实所见如此.",
